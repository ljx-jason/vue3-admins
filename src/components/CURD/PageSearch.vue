<template>
  <el-card v-show="visible" v-hasPerm="[`${searchConfig.pageName}:query`]" shadow="never" class="mb-2.5">
    <el-form ref="queryFormRef" :model="queryParams" :inline="true">
      <template v-for="(item, index) in formItems" :key="item.prop">
        <el-form-item v-show="isExpand ? true : index < showNumber" :label="item.label" :prop="item.prop">
          ...
          <component :is="componentMap[item?.type ? item?.type : 'input']" v-model.trim="queryParams[item.prop]"
            v-bind="item.attrs" :config="item.attrs" v-on="item.events || {}">
      ...
      <el-form-item>
        ...
        <!-- 展开/收起 -->
        <el-link v-if="isExpandable && formItems.length > showNumber" class="ml-3" type="primary" :underline="false"
